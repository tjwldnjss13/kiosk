...
def detect(args):
    ...
    # Initialize model
    yolo = YOLOV2PedFaceLite(in_size=(416, 416)).to(device)
    ...
    writer = cv.VideoWriter('./videos/' + str(now) + '.avi', fourcc, 20.0, (640, 480), True)
    ...
    while cap.isOpened():
        ...
        cv.putText(img_result, str(check_time), (10, 30), cv.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv.LINE_AA)
        img_write = cv.resize(img_result, (640, 480), interpolation=cv.INTER_CUBIC)
        writer.write(img_write)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--yolo_ckpt', required=False, type=str, default='./models/weights/YOLOV2PedFaceLite_temp.ckpt')
    ...
    parser.add_argument('--person_conf', required=False, type=float, default=.9)
