...
from models.model4.tiny_darknet_cbam import TinyDarknetCBAM2
...
def test_person(args):
    ...
    def get_results(predict, annotation):
        ...
        return ious_val[:, 0].detach().cpu().numpy(), truths.detach().cpu().numpy(), pred_score.detach().cpu().numpy()
    ...
    # model = TinyYOLOV3PedFace2(in_size, num_classes, False).to(device)
    model = TinyDarknetCBAM2(1, device).to(device)
    ...
    for split_idx in range(num_dset_split):
        ...
        for i, (img, ann) in enumerate(loader):
            ...
            pred = model(img)
            box_per_cls, conf_per_cls = model.final_result(pred, args.conf_thresh, args.nms_thresh)
            if len(box_per_cls[0]) == 0:
                outputs = None
            else:
                outputs = [convert_box_from_xyxy_to_yxyx(box_per_cls[0]), conf_per_cls[0]]
            ...
            # print(ann['bbox'])
            ap = average_precision(truth_list, score_list, num_tar)
            print(f'(AP : {ap:.3f}) (# True IoU : {num_true_iou/(num_total_iou+1e-12):.3f})', end='\r')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--yolo_ckpt', required=False, type=str, default='./models/weights/TinyDarknetCBAM2(ped)_0.520iou.ckpt')
    parser.add_argument('--agnet_ckpt', required=False, type=str, default='./models/weights/AGNetSuperSub_temp.ckpt')
    parser.add_argument('--conf_thresh', required=False, type=float, default=.1)
    parser.add_argument('--nms_thresh', required=False, type=float, default=.3)
    parser.add_argument('--iou_thresh', required=False, type=float, default=.5)
